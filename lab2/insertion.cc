...
void scootOver( int jj ) {
  for(int kk = y_size; kk > jj; kk-- ){ // changed "kk = y_size - 1" to "kk = y_size"
    y[kk] = y[ kk - 1 ];
  } 
}

void insert( int xx ) {
  if( y_size == 0 ) { // changed = to ==
    y[0] = xx;
    return;
  }
  ...
  y[y_size] = xx; // if inserting number is not bigger than any other number put it at the end
}
